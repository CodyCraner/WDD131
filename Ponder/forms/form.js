...
form.addEventListener("submit", function (event) {
  ...
  if(type === "one" && selectedCampuses.length == 0){
    output.textContent = "You promised you would come to one campus " + 
    ",like Samson of old you have broken the covenant, please select one campus";
    return;
  }
...
if(type === "many" && note.length == 0){
    output.textContent = "Error, please contact admin";
    return;
  ...
  if(type === "many" && selectedCampuses.length < 2){
    output.textContent = "First Name is incorrect."
    return;
  }
